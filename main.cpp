...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	swapChainDesc.BufferCount = DXGI_USAGE_RENDER_TARGET_OUTPUT;//描画のターゲットとして利用する
	...
	//Shaderをコンパイルする
	//ファイル名の3Dが小文字なのにここが大文字になっている
	IDxcBlob* vertexShaderBlob = CompileShader(L"Object3D.VS.hlsl", L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(vertexShaderBlob != nullptr);
	IDxcBlob* pixelShaderBlob = CompileShader(L"object3D.PS.hlsl", L"vs_6_0", dxcUtils, dxcCompiler, includeHandler);
	...
	hr = device->CreateCommittedResource(&uplodeheapProperties, D3D12_HEAP_FLAG_NONE, &vertexResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&vertexResource));
	assert(SUCCEEDED(hr));
	//頂点バッハビューを作成する
